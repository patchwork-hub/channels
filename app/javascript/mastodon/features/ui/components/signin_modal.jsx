...
const SignInModal = () => {
  const [loginWithChannel, setLoginWithChannel] = useState(false);
  ...
  async function handleSubmit() {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/oauth/token?create_channel_feed=true`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            username: email,
            password,
            client_id: 'biNVYrE8eOYOvUivWVsJb5i41kDCUUvMLWpL32XTaZQ',
            client_secret: '55v0vm8Va5wDme3EMi6DhmY1KXfKl74nXXURpT_Jbmo',
  ...
  return (
    <div className='modal-root__modal signin-modal-wrap'>
      <h1
        style={{
          fontSize: 30,
          fontWeight: 700,
      ...
      >
        If you already have an account with us then choose an option below to
        login and join in the conversation.
      </p>
      ...
      {loginWithChannel ? (
        <div style={{ width: '100%' }}>
          ...
          >
            <p style={{ color: '#D2D2FF', paddingBottom: 10 }}>
              Email Address{' '}
              <span
                style={{
                  color: 'red',
          ...
          <div>
            ...
            >
              {isLoading ? <span className='loader' /> : <span>Login</span>}
            </button>
          </div>
        </div>
      ) : (
        <>
          <button
            onClick={() => setLoginWithChannel(true)}
            style={{
              alignSelf: 'stretch',
              borderRadius: 8,
              border: 0,
          ...
          >
            <div
              style={{
                flexGrow: 1,
                borderBottom: '1px solid rgba(220, 224, 235, 0.4)',
                height: 0,
              }}
            ></div>
            <p
          ...
          >
            ...
            <a style={{ textDecoration: 'underline' }}>Create one now</a>
          </p>
        </>
      )}
    </div>
